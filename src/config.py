...
# Во время генерации модель кормит себя собственными предыдущими точками.
# Если обучать только с teacher forcing = 1.0, она может хорошо реконструировать
# датасет, но разваливаться в свободном режиме. Слишком резкий переход портит
# уже выученную реконструкцию, поэтому scheduled sampling включаем мягко:
# 0.98 означает, что только в 2% шагов следующая точка берётся из предсказания.
teacher_forcing_ratio = 0.98
scheduled_sampling_mode = "mean"  # "argmax", "mean" или "sample".
